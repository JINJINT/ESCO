...
escoSimGeneMeans <- function(sim, verbose) {
    ...
    batch.means.cell <- matrix(1, ncol = nCells, nrow = nGenes) * gene.means
...
escoSimPathDE <- function(params, verbose) {
...
escoSimCellMeans <- function(sim, verbose) {
  params = metadata(sim)$Params
  ...
  basecells.props <- t(t(basecells.means) / colSums(basecells.means))
  basecells.means <- t(t(cells.props)*cell.lib.sizes)
...
escoSimTrueCounts <- function(sim, type, verbose) {
    ...
    if(!withcorr){
      ...
      if(type %in% c("groups","tree")){
        ...
        if(length(corr)!=0){
          if(length(corr)!=nGroups+1){
            ...
            for(idx in 1:length(sort(unique(groups)))){
                ordsim = sort(genemeans[match(as.character(gene.names[which(rowData(sim)$GeneGroup==idx)]), gene.names)], index.return  = TRUE)$ix
      ...
      true.count <- foreach(i = 1:total, .combine = cbind, .options.snow = opts) %dopar% {
        return(mulpo(i))
      }
      stopCluster(cl) 
    }
...
Observed_Counts <- function(sim, trial, verbose, protocol = "UMI", nbatch=1){
  ...
  for(alpha_mean in alpha_mean_vec){
    for(depth_mean in depth_mean_vec){
      ...
      opts <- list(progress = progress)
      observed_counts <- foreach(i = c(1:total), .options.snow = opts) %dopar% {
