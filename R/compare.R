...
#'         \describe{
#'             \item{\code{Means}}{violin of mean distribution.}
#'             \item{\code{Variances}}{violin of variance distribution.}
#'             \item{\code{MeanVar}}{Scatter plot with fitted lines showing the
#'             mean-variance relationship.}
#'             \item{\code{LibrarySizes}}{violin of the library size
#'             distribution.}
#'             \item{\code{ZerosGene}}{violin of the percentage of each gene
#'             that is zero.}
#'             \item{\code{ZerosCell}}{violin of the percentage of each cell
#'             that is zero.}
...
#' @importFrom ggplot2 ggplot aes_string geom_point geom_smooth geom_violin
#' scale_y_continuous scale_y_log10 scale_x_log10 xlab ylab ggtitle theme_minimal scale_colour_manual scale_fill_manual element_text
#' @import ggplot2
...
compareSCEs <- function(sces, point.size = 0.1, point.alpha = 0.1,
  ...
  if (!is.null(colours)) {
    ...
    colours <- c("black", "orange", "steelblue",  "olivedrab", "firebrick", "chocolate2", "skyblue", "olivedrab", "aquamarine4")[1:length(sces)]
  }
  
  for (name in names(sces)) {
    sce <- sces[[name]]
    rowData(sce)$Dataset = rep(name, nrow(assays(sce)$counts))
    colData(sce)$Dataset = rep(name, ncol(assays(sce)$counts))
  ...
  mean.zeros <- ggplot(features,
    ...
    geom_point(size = point.size, alpha = point.alpha) +
    scale_x_log10() +
    #labels = scales::comma_format(accuracy = 3)
    scale_colour_manual(values = colours) +
    scale_fill_manual(values = colours) +
    xlab("Mean count") +
...
#'         \describe{
#'             \item{\code{Means}}{violin of mean differences.}
#'             \item{\code{Variances}}{violin of variance differences.}
#'             \item{\code{MeanVar}}{Scatter plot showing the difference from
#'             the reference variance across expression ranks.}
#'             \item{\code{LibraeySizes}}{violin of the library size
#'             differences.}
#'             \item{\code{ZerosGene}}{violin of the differences in the
#'             percentage of each gene that is zero.}
#'             \item{\code{ZerosCell}}{violin of the differences in the
...
#' @return List containing the combined datasets and plots.
#' @examples
#' sim1 <- splatSimulate(nGenes = 1000, batchCells = 50)
#' sim2 <- simpleSimulate(nGenes = 1000, nCells = 50)
#' difference <- diffSCEs(list(Splat = sim1, Simple = sim2), ref = "Simple")
#' names(difference)
#' names(difference$Plots)
#' @importFrom ggplot2 ggplot aes_string geom_point geom_violin xlab ylab
...
diffSCEs <- function(sces, ref, point.size = 0.1, point.alpha = 0.1,
  ...
  if (!is.null(colours)) {
    ...
    colours <- c("black", "orange", "steelblue", "firebrick", "chocolate2", "skyblue", "olivedrab", "aquamarine4")[1:length(sces)]
...
makeCompPanel <- function(comp, title = "Comparison",
  ...
  for (plot in paste0("p", c(1, 2, 4, 5, 6))) {
    plots[[plot]] <- plots[[plot]] + theme(plot.title = element_text(size = 30),
                                            axis.title.x = element_blank(),
...
makeDiffPanel <- function(diff, title = "Difference comparison",
  ...
  # Remove x-axis title from some plots
  for (plot in paste0("p", c(1, 3, 6, 7, 8, 10))) {
    plots[[plot]] <- plots[[plot]] + theme(plot.title = element_text(size = 30),
                                           axis.title.x = element_blank(),
                                           axis.text.x = element_text(size = 25, angle = 45, hjust = 1, vjust = 1))
  }
...
#' @examples
#' \dontrun{
#' sim1 <- splatSimulate(nGenes = 600, batchCells = 20)
#' sim2 <- simpleSimulate(nGenes = 600, nCells = 20)
