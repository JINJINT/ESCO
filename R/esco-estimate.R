...
#' params
#' @export
splatEstimate <- function(counts, dirname, rho = FALSE, group = FALSE, metacell = NULL, cellinfo = NULL,params = newSplatParams()) {
...
#' @export
#' 
splatEstimate.SingleCellExperiment <- function(counts, dirname, rho = FALSE, group = FALSE, metacell = NULL, cellinfo = NULL,
...
#' @export
splatEstimate.matrix <- function(counts, dirname, rho = FALSE, group = FALSE, metacell = NULL, cellinfo = NULL, params = newSplatParams()) {
  ...
  if(!group){
    ...
    params <- splatEstLib(counts, norm.counts, params)
    params <- splatEstMean(norm.counts, counts, params)
    params <- splatEstOutlier(norm.counts, params)
    params <- splatEstBCV(counts, norm.counts, params)
   
    
    # if(rho){
    #   params <- splatEstRho(norm.counts, metacell = NULL, params)
    # }
    ...
    params <- splatEstGroupMean(nonDE.normcounts, DE.normcounts, degenes, cellinfo, params)
    params <- splatEstBCV(counts, norm.counts, params)
    #params <- splatEstOutlier(norm.counts, params)
    params <- splatEstGroupOutlier(nonDE.normcounts, DE.normcounts, degenes, cellinfo, params)
    de <- splatEstDE(nonDE.normcounts, DE.normcounts, degenes, cellinfo, params)
    de.facLoc = de[[1]]
    de.facScale = de[[2]]
    de.rank = de[[3]]
    de.facrank = de[[4]]
    ...
    params <- splatEstDropout(counts, params)
    #params <- setParams(params, dropout.type = "experiment")
    
    # if(rho){
    #   params <- splatEstRholist(norm.counts, cellinfo = cellinfo, degenes = degenes, housegenes = housegenes, params)
    # }

    params <- setParams(params, nGenes = nrow(counts), batchCells = ncol(counts), deall.prob = deall.prob, house.prob = house.prob)
    return(params)
  }
}
...
splatEstLib <- function(counts, norm.counts, params) {
...
#' Estimate Splat group mean parameters
#'
...
#'
#' @return SplatParams object with estimated values.
splatEstMean <- function(normcounts, counts, params) {
...
#'
#' @return SplatParams object with estimated values.
splatEstGroupMean <- function(nonDE.normcounts, DE.normcounts, degenes, cellinfo, params) {
    ...
    params <- setParams(params, mean.shape = unname(fit$estimate["shape"]),
                        mean.rate = unname(fit$estimate["rate"]), mean.dens = density(lnorm))
...
#'
#' @importFrom stats shapiro.test
splatEstGroupLib <- function(counts, cellinfo, params) {
  lib.norm = c()
  lib.loc = c()
  lib.scale = c()
...
#' Estimate Splat expression DE parameters
#'
...
#'
#' @return SplatParams object with estimated values.
splatEstDE <- function(nonDE.normcounts, DE.normcounts, degenes, cellinfo, params) {
  de.rank = list()
  de.facrank = list()
  means <- rowMeans(nonDE.normcounts)
  for(idx in 1:length(unique(degenes$clusts))){
...
#' Estimate Splat Group expression outlier parameters
#'
#' Parameters are estimated by comparing means of individual genes to the
#' median mean expression level.
#'
...
#'
#' @return SplatParams object with estimated values.
splatEstGroupOutlier <- function(nonDE.normcounts, DE.normcounts, degenes, cellinfo, params) {
    means <- rowMeans(nonDE.normcounts)
    for(idx in 1:length(unique(degenes$clusts))){
      marker = degenes$genes[which(degenes$clusts==idx)]
      cells = colnames(DE.normcounts)[which(cellinfo!=levels(cellinfo)[idx])]
...
#'
#' @return SplatParams object with estimated values.
splatEstBCV <- function(counts, norm.counts, params, cellinfo = NULL){
...
#' Estimate Splat correlation matrix using meta data or the copula method
#'
#' @importFrom stats shapiro.test
splatEstCorr <- function(counts, cellinfo, params, method = c("meta", "copula")) {
